...
final class UserController: BaseController {
    ...
    func createUser(_ req: Request, data: CreateUser) throws -> Future<DataResponse<PublicUser>> {
        ...
        return req.dispatch { request in
            ...
            let newUser = try User(id: nil, firebaseId: firebaseUser.sub, name: data.name, color1: data.color1, color2: data.color2, currency: 0, mmr: 1000, fcmId: data.fcmId).save(on: req).wait()
